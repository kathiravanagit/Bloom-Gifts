...
function seed() {
  ...
  if (countRow.c > 0) {
    ...
    const insertProduct = db.prepare(`
      INSERT INTO products (category, name, slug, tagline, description, base_price, image, is_customizable, badge)
      VALUES (@category, @name, @slug, @tagline, @description, @base_price, @image, 1, @badge)
    `);
    const insertOption = db.prepare(`
      INSERT INTO product_options (product_id, group_name, group_type, option_name, extra_price, is_default, sort_order)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
      ...
      for (const p of items) {
        ...
        p.options.forEach((group) => {
          group.items.forEach((item, idx) => {
            insertOption.run(info.lastInsertRowid, group.group_name, group.group_type, item[0], item[1], item[2], idx);
          });
        });
      }
    });
  ...
  const componentRow = db.prepare('SELECT COUNT(*) AS c FROM hamper_components').get();
  if (componentRow.c > 0) {
    console.log('Hamper components already seeded, skipping.');
  } else {
    const insertComponent = db.prepare(`
      INSERT INTO hamper_components (category, name, price, description, sort_order)
      VALUES (?, ?, ?, ?, ?)
    `);
    const insertComponents = db.transaction((items) => {
      items.forEach((c, idx) => {
        insertComponent.run(c.category, c.name, c.price, c.description, idx);
      });
    });
    insertComponents(hamperComponents);
    console.log(`Seeded ${hamperComponents.length} hamper components.`);
  }

  const adminRow = db.prepare('SELECT COUNT(*) AS c FROM admin_users').get();
  if (adminRow.c === 0) {
